...
**그 실행이 얼마를 썼는지 (TH-D21).** `th protocol status`는 stage별 실행 기록 아래에
사용량 요약을 함께 출력합니다: 총 소요 시간, agent별·model별 시간 분해, 그리고 워커가
보고한 토큰 수와 비용.

```text
[protocol] ----- 사용량 -----
  실행된 stage 2개 · 총 60.0초
  agent별 시간: codex 48.0s, claude 12.0s
  토큰: 합계 1,000 · 입력 900 · 출력 100
  워커 보고 비용: $0.2500 USD
  ⚠ 위 토큰/비용은 실행된 2개 stage 중 1개만 반영합니다 ...
```

시간은 하네스가 직접 재므로 항상 완전하지만, **토큰과 비용은 워커 CLI가 스스로 보고한
값**입니다. antigravity는 평문을 출력하고 카운터를 전혀 보고하지 않으므로, antigravity가
섞인 실행은 토큰 수가 완전할 수 없습니다. 그래서 합계가 실행된 stage 전부를 반영하지
않으면 위처럼 몇 개만 반영했는지 경고합니다 — 부분 합계를 전체 비용으로 읽는 것이 이
기능에서 가장 위험한 오해이기 때문입니다. 하네스는 모델 가격표를 갖지 않으므로 토큰을
돈으로 환산하지 않습니다.
