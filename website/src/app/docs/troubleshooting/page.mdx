...
Workers are separate CLIs with their own installation and auth. Confirm the binary is installed and that you can run it standalone (e.g. `codex`, `claude`, `grok`, `agy`). Each worker uses its **own native auth** — the harness does not forward the coordinator's API key to workers unless you explicitly configure `provider_env` (see [Providers & Auth](/docs/providers#routing-workers-through-openrouter)). Restrict a run to only the workers you have with `--agents codex,antigravity`. For Grok, set `XAI_API_KEY` or run `grok login`.
