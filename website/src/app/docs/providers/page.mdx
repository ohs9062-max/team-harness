...
### Antigravity via OpenRouter

Not supported by the upstream `agy` CLI — it authenticates directly against its own account with no OpenAI-compatible base-URL mode. The harness ships no recipe.
