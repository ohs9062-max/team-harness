...
- **`prompt_flag` / `prompt_position`** — the task prompt is appended at the tail of argv by default. Set `prompt_flag = "-p"` if the prompt is introduced by a flag (like `grok -p`), or `prompt_position = "after_command"` if the CLI wants it earlier.
...
The new type appears automatically in the coordinator's `spawn_agent` tool. Session ids can be captured via a `[agents.<name>.session_capture]` sub-table with `strategy`, `match`, and `field_path` (see the codex/claude examples).
...
| antigravity | (not supported upstream)                  | —                          |
